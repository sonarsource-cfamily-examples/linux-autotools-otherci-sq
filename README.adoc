...
. Run `sonar-scanner` specifying `-Dsonar.cfamily.compile-commands=bw-output/compile_commands.json` (with `bw-output` being the same directory as the one passed to Build Wrapper as `--out-dir`)
