...
. Wrap your clean build command with the Build Wrapper. E.g.: `build-wrapper-linux-x86-64 --out-dir bw-output make clean all` (with `bw-output` being the same directory that the one set in `sonar-project.properties`)
. Run `sonar-scanner`

You can take a look at the link:sonar-project.properties[sonar-project.properties] and link:build-analyze-project.sh[build-analyze-project.sh] (script that downloads the `sonar-scanner` and the Build Wrapper before compiling and analyzing the project ) to see it in practice. Please be aware that the SonarQube server URLs and the `sonar.login` are not completed in these examples.
